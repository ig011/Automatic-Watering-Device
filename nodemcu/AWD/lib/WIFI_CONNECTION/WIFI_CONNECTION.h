...
void WifiConnection_Init(const char *STASSID, const char *STAPSK, const uint16_t blink_delay = 300, const uint8_t max_tries = 20)
{
    ...
    while (WiFi.status() != WL_CONNECTED && (attempts_counter < max_tries))
    {
        ...
        if (attempts_counter >= max_tries)
        {
            WiFi.mode(WIFI_OFF);
            digitalWrite(LED_BUILTIN, LOW);
            Serial.println(WiFi.softAP("AWD-Device") ? "Access Point has been created" : "Access Point init failed!");
        }
    }
    ...
    if (WiFi.status() == WL_CONNECTED)
    {
        ...
        Serial.println("Connected to the network: " + String(STASSID));
...
void WifiConnection_Connect(const char *STASSID, const char *STAPSK)
{
}
